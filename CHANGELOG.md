...
- **Evaluation trace (`t`).** Shows the path a log row took through the policy in Azure Firewall's real processing order — Threat Intelligence, then DNAT → Network → Application passes, inherited policy first, by priority, stopping at the logged rule. Every criterion (source, destination, port, protocol) is marked ✓ / ✗ / ? per rule, so near misses on *no rule matched* rows show exactly what failed. `Enter` jumps to the rule in the Policy tab. Inherited parent policies are fetched and shown.
