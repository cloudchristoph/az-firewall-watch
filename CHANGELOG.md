...
- **Small terminals.** Below 120 columns the fields and the trace become two tabs inside the dialog (*Fields* / *Policy trace*, `Tab` switches, the trace tab opens first); below 40 rows the selection detail gives its rows to the tree, below 30 the header keeps only the rule name. The tree keeps its selected line in view through every layout change, and every pane scrolls on its own. At 80×24 the trace was unusable before and is readable now; the screenshots in the PR show all three sizes in both modes.
...
- **The `Rule Def.` line is gone from the row detail dialog.** It squeezed the whole rule into one ad-hoc sentence (`Http, Https  80, 443  from … to *`), wrapped badly, and repeated what the trace beside it shows criterion by criterion; the full definition is one `Enter` away in the Policy tab. The one thing it carried that the trace did not, the names of inserted HTTP headers, now sits under the rule in the trace tree.
