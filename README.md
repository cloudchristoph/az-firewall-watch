...
It is **on by default** and saved as `POLICY_CONTEXT=on|off`. Choose *Disable* if
...
| `POLICY_CONTEXT`                  | `on` / `off` — policy context via Azure Resource Manager (tabs, trace, cache). `--policy-context` / `--no-policy-context` override it for one run. If the key is missing, the viewer asks once at start-up and saves the answer | `on`       |
<!-- markdownlint-enable MD060 -->
...
check, the ARM client and policy context (against canned ARM payloads),
