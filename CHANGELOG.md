...
- 836 tests (583 in 0.5.1); every new fact has tests for the field being present, absent and unreadable, the explicit-proxy and TLS-inspection cases are modelled on the lab's real records, and the row detail dialog is rendered at three terminal sizes in both modes.
...
- **Keys in the dialog.** `Enter` folds and unfolds a node and nothing else; it used to open the Policy tab from an unfolded rule, which made the second press a surprise. `p` opens the selected rule in the Policy tab, `a` toggles between the focused and the full tree, `Esc` and `q` close. The Close button is gone; the footer lists the keys that apply.
