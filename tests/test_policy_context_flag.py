"""The POLICY_CONTEXT feature flag: config resolution, .env persistence, viewer behaviour."""
...
def test_env_value_is_explicit(value, enabled):
    assert policy_context_setting([], {"POLICY_CONTEXT": value}) == (enabled, True)


@pytest.mark.parametrize("environ", [{}, {"POLICY_CONTEXT": ""}, {"POLICY_CONTEXT": "   "}])
def test_missing_value_defaults_on_but_not_explicit(environ):
    assert policy_context_setting([], environ) == (True, False)


def test_cli_flags_override_environment():
    assert policy_context_setting(["main.py", "--no-policy-context"], {"POLICY_CONTEXT": "on"}) == (False, True)
    assert policy_context_setting(["main.py", "--policy-context"], {"POLICY_CONTEXT": "off"}) == (True, True)
    assert policy_context_setting(["main.py", "--policy-context"], {}) == (True, True)
...
def test_write_env_records_policy_context(tmp_path: Path):
    env = tmp_path / ".env"
    write_env(env, CONN)
    assert _values(env)["POLICY_CONTEXT"] == "on"
    write_env(env, CONN, policy_context=False)
    assert _values(env)["POLICY_CONTEXT"] == "off"
    write_env_entra(env, "ns.servicebus.windows.net", "h", policy_context=False)
    assert _values(env)["POLICY_CONTEXT"] == "off"
...
def test_set_env_value_replaces_in_place(tmp_path: Path):
    ...
    set_env_value(env, "POLICY_CONTEXT", "off")
...
def test_set_env_value_appends_to_pre_flag_env(tmp_path: Path):
    """A 0.4.x .env has no POLICY_CONTEXT line; the decision is appended with its comment."""
    env = tmp_path / ".env"
    env.write_text(f"EVENT_HUB_CONNECTION_STRING={CONN}\nEVENT_HUB_START_POSITION=latest\n", encoding="utf-8")
    set_env_value(env, "POLICY_CONTEXT", "on")
    text = env.read_text(encoding="utf-8")
    assert text.startswith(f"EVENT_HUB_CONNECTION_STRING={CONN}\n")
    assert text.endswith("POLICY_CONTEXT=on\n")
    assert "# POLICY_CONTEXT=on reads" in text
...
# ── viewer with policy context off ───────────────────────────────────────────────
...
async def test_off_means_logs_only_and_no_arm(arm_calls):
    app = FirewallLogApp(policy_context=False)
...
async def test_off_shows_no_notice(arm_calls):
    ...
    async with app.run_test(size=(160, 45)) as pilot:
        await pilot.pause()
        assert not isinstance(app.screen, PolicyContextNoticeDialog)


async def test_on_without_notice_is_silent(arm_calls):
    app = FirewallLogApp(policy_context=True, policy_context_notice=False)
    async with app.run_test(size=(160, 45)) as pilot:
        await pilot.pause()
        assert not isinstance(app.screen, PolicyContextNoticeDialog)
...
async def test_notice_without_env_file_does_not_write(tmp_path: Path, arm_calls):
    app = FirewallLogApp(policy_context=True, policy_context_notice=True, env_file=tmp_path / "missing.env")
    async with app.run_test(size=(160, 45)) as pilot:
        await wait_until(pilot, lambda: isinstance(app.screen, PolicyContextNoticeDialog))
        await pilot.pause()
        await pilot.click("#btn-disable")
