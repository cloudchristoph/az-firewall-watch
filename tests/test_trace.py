...
# ── IPv6 flows (dual-stack firewall) ─────────────────────────────────────────
...
def test_ipv6_flow_matches_an_ip_group_holding_a_v6_prefix():
...
def test_ipv6_flow_through_lab_policy_reports_the_address_as_the_near_miss():
    ...
    t = build_trace(TCP443_V6, LAB, GROUPS, None)
...
def test_ipv6_destination_is_checked_as_an_address_not_as_an_fqdn():
    """A destination that reaches the trace as ``dst_ip`` goes through the address check. A
    network rule with only FQDN targets then answers *unknown* (what the FQDN resolved to is
    not knowable here), never a confident miss from the FQDN branch."""
    rule = net("r", destination_addresses=[], destination_fqdns=["ifconfig.me"])
...
def test_garbled_ipv6_source_is_not_applicable_rather_than_a_verdict():
    """What a first-colon split would produce ('fd10' from 'fd10:2:0:2::10:51000'): not an
    address, so the source check must say n/a instead of match or miss."""
