...
- **NAT gateway on the firewall subnet.** The Networking block listed the firewall's public IPs and thereby answered "which address does the far end see" wrongly whenever a NAT gateway sits on the subnet: traffic routed straight to the internet then leaves with the gateway's public IPs, while DNAT and the firewall's management traffic stay on the firewall's own. The viewer reads no route tables, so both lines say what they cannot know: a route to an NVA or a virtual network gateway bypasses the NAT gateway, and with forced tunneling internet-bound traffic need not leave with any firewall public IP at all. The subnet is read anyway for the `AzFw.<n>` rendering; its `natGateway` reference is one field more, followed by an optional `GET` on the gateway for its public IPs. A Virtual WAN hub firewall gets *not supported* rather than an empty line, an unreadable gateway is named as unreadable.
...
- **Customer-controlled maintenance window.** When an instance goes away for maintenance the firewall resets existing connections, and the RST burst in the log looks like an incident. The Instance block now reads the `Microsoft.Maintenance` assignment under the firewall and its configuration, and shows the daily window with duration and time zone as Azure wrote them, with the caveat the docs attach: the window covers guest OS and service updates, while host updates and urgent security fixes can fall outside it. An unregistered provider means there is nothing to read, not an error; an assignment list that cannot be read is reported as unknown rather than as no window; a configuration that cannot be read is named as assigned but unreadable. The expiration is shown with its time, and a window counts as expired only once a full local day has passed, because Azure writes the expiration in the window's own time zone and the viewer does not resolve Windows zone names.
- **HTTP header insertion on application rules.** A rule that inserts headers writes into other people's traffic, and the Policy tab showed it as if it did nothing of the sort. The rule detail now lists the inserted headers by name (`httpHeadersToInsert`, verified against a real policy: the entries are `headerName` and `headerValue`), together with the rule's TLS inspection and a line that says where the headers really land: HTTP only on Standard and Basic, HTTP only on Premium as long as the rule does not terminate TLS, HTTP and inspected HTTPS otherwise. The line names only traffic the rule can match: an HTTPS-only rule that cannot be inspected inserts into nothing, and says so rather than "HTTP only". Header values can carry tokens and tenant ids, so they are hidden behind `v` and hidden again on every refresh; the trace names the inserted headers under the rule, never the values.
- **The property bag as proper rows.** The Instance block used to dump `additionalProperties` as `EnableFatFlowLogging=true`. The switches Azure keeps there now have rows of their own: *Fat flow logging* (off when not set, which is Azure's default, and the usual reason the FatFlow category never shows up), plus *Active FTP*, *DNS flow trace* and the classic DNS proxy and SNAT settings when present. Keys the viewer does not know stay visible under *Additional*, so nothing is lost.
- **The management configuration no longer implies forced tunneling.** A management IP configuration is what forced tunneling needs and what the Basic SKU requires, not proof that a route tunnels anything, and the viewer reads no route tables. The Networking block says so instead of *forced tunneling*.
...
- 953 tests (583 in 0.5.1); every new fact has tests for the field being present, absent and unreadable, the explicit-proxy and TLS-inspection cases are modelled on the lab's real records, and the row detail dialog is rendered at three terminal sizes in both modes. The setup wizard has a test for every Back, Cancel and error branch, and an optional live suite (`tests/live/test_live_wizard.py`, see the development notes) drives all six of its paths against a real subscription, the deploy path included, and removes what it created.

### Changed

- **Status bar rebuilt around three sources.** The bar used to mix the Event Hub connection, the event counters and the ARM-backed policy context in two free-text fields, so *Connected* did not say connected to what, and a quiet connection looked the same as one that delivers nothing. It now has one segment per source, each with a glyph (`●` fine, `◐` in progress, `○` off or idle, `✖` needs you), a label and a state word: `▶ LIVE │ ● EH connected │ ● Events receiving · 1079/5113 shown · last 2s ago │ ● Context loaded 35m ago`. The new **Events** segment reports *waiting*, *receiving* or *idle* with the time since the last record, which is the fastest way to spot switched-off diagnostic settings or a wrong consumer group. On a narrow terminal the details go first, then the labels.
- **Context segment shows state, not stock data.** The policy tier and the number of IP groups left the bar; they never change during a session and already live in the Firewall, Policy and IP Groups tabs. What stays is whether the context is loaded and how old the cached policy is, now spelled out as `loaded 35m ago` instead of a bare `cache 35m`. The bar ages the cache itself at render time, so the age can no longer stick at an old value.
- **Hints moved out of the bar.** One-off messages (*Nothing to refresh yet*, *Policy context is off*, a failed refresh, connection error texts) are notifications now, and the last Event Hub error is the bar's tooltip. Before, such a hint overwrote the context segment and stayed there until the next re-fetch. The reason for a missing trace sits in the dialog, see *Fixed*.
- **One word for the ARM data: policy context.** `Ctrl` + `R` is *Refresh context* (was *Refresh metadata*), the status bar's segment is *Context*, and the dialog's note says *policy context* where it said *firewall metadata*. Same key, same behaviour; *metadata* is gone as a word the user reads.
- **The row detail dialog was rebuilt around the logged rule.** It used to be two things glued together: every field of the record on the left, and on the right a tree in which every line had the same weight, every collection carried a `nearest miss` suffix, and the rule the firewall actually logged sat somewhere in the middle. Now the header says what happened (connection, protocol and logged action; with policy context a second line with the trace's outcome and the cache age), the fields sit in groups (*Connection*, *Inspection*, *Groups*, *Rule*) with local and UTC time on one line, and the tree opens focused: the logged rule selected and in view, the collections rejected before it folded into `n preceding collections   all ✗`, the ones never reached into `n not evaluated`, and a `?` anywhere inside a fold keeps that fold open. Tree lines are status, priority and name; the criteria moved to a detail under the tree that follows the selection and shows every check with the observed value and the rule's expectation, whole names included. Header values of an inserting rule stay in the Policy tab. The dialog works the same without policy context: then it is the fields, the reason there is no trace, and `Esc close`.
- **Keys in the dialog.** `Enter` folds and unfolds a node and nothing else; it used to open the Policy tab from an unfolded rule, which made the second press a surprise. `p` opens the selected rule in the Policy tab, `a` toggles between the focused and the full tree, `Shift` + `↑` / `↓` scroll the selection detail under the tree without leaving it, `Esc` and `q` close, and so does `Enter` on a row without a trace, where it had nothing else to do. The footer lists the keys that apply and keeps a compact Close button for the mouse; it never takes the focus.
- **The frame carries the verdict.** The dialog's border is green, red or yellow for the logged allow, deny or dnat, with the action and the rule in its title tab (`ALLOW · allow-web`), so the outcome is read before a single line inside. The header's second line then says where the rule sits (`✓ matched in group » collection`) rather than repeating the name. Fields, tree and selection detail sit in titled frames (*Fields*, *Policy evaluation*, *Selection*); the selection frame keeps one height per trace instead of growing and shrinking with the cursor, and on short terminals the frames fold to their top rule. The log table stays visible behind the dialog, one step further back.
- **Small terminals.** Below 120 columns the fields and the trace become two tabs inside the dialog (*Fields* / *Policy trace*, `Tab` switches, the trace tab opens first); below 40 rows the selection detail gives its rows to the tree, below 30 it keeps only its name lines. The header's outcome line is measured against the dialog's width: when it would wrap it drops the group and collection, then the cache age, and keeps the rule name. The tree keeps its selected line in view through every layout change, and every pane scrolls on its own. At 80×24 the trace was unusable before and is readable now; the screenshots in the PR show all three sizes in both modes.
- **The Firewall tab reads as columns.** Every row of the Instance and Policy blocks used to trail its detail three spaces behind the value, so the details started wherever the value happened to end. A state word (`on`, `off`, `Alert`, `default`, `autoscaling`) now sits in a column of its own and every detail starts in the same place; a value too long for that column keeps its line and the detail moves under it. The Networking table puts the private and public addresses first and the configuration name last, since the name is a label and the addresses are what one came for.
- **Logging as a matrix, with one coverage line.** The block used to list diagnostic settings by name with a count of categories, and a *Not to Event Hub* line below. It is now a matrix of categories against targets (Event Hub, Log Analytics, Storage), the viewer's categories first, with the setting names in a legend, and a single **Event Hub coverage** line for the hub this viewer reads from: green *complete*, or yellow *incomplete* with the missing names. Missing means missing for this firewall: no IDPS log is expected without Premium and IDPS on, no DNS log without the proxy, no fat flow log without the switch, no threat intel log with the mode off. Flow trace depends on a subscription feature registration the viewer does not read, so it is named when absent and never counted. An empty threat intelligence mode counts as on (Azure's default is Alert), only an explicit Off takes the category out.
- **Quieter colours.** A miss is the normal case for most rules, so `✗` is dim rather than red; `?` stays the loudest mark, not-evaluated collections are dimmer still, the selection is a light bar rather than the saturated primary. `deny` stays loud, `allow` quiet, and the legend renders through the same path as the tree so the colours agree. The dialog's own frame is thin and `$panel` instead of blue.

### Fixed

- **The reason for a missing trace sits in the dialog, not in the status bar.** Opening a FlowTrace, FatFlow, DNS or IDPS row used to write *no policy evaluation for FatFlow rows* into the status bar, where it replaced the policy and cache state and stayed there until the next refresh, and where it read as if the firewall had skipped the policy. The dialog now says it itself, under the row's fields: *No rule decision in this log*, with what that category records instead (FatFlow the top flows by rate, FlowTrace the handshake and flags, DNS proxy rows the query and its answer, IDPS a signature hit). A row opened before the policy context is loaded says *Policy trace not available* the same way; a loaded context without a policy (unreadable, or a firewall on classic rules) says that instead of *not loaded yet*, since waiting changes nothing there. The status bar keeps its Context segment.
- **A DNAT rule against a named destination misses on the destination, not the port.** An application-rule row names its destination; a DNAT rule matches traffic addressed to the firewall's own public IP. The trace could not compare a name with that address, marked the destination as not in the log, and blamed the port instead (`nearest miss: port`), sending the reader to look at a port that was never the point. The destination is now the miss, with the reason: `www.example.com is a name; DNAT matches 72.144.131.50`.
- **The `Rule Def.` line is gone from the row detail dialog.** It squeezed the whole rule into one ad-hoc sentence (`Http, Https  80, 443  from … to *`), wrapped badly, and repeated what the trace beside it shows criterion by criterion; the full definition is one `p` away in the Policy tab. The one thing it carried that the trace did not, the names of inserted HTTP headers, now sits under the rule in the trace tree.
- **One yellow for every `?`.** The trace tree renders its labels as Rich text through the terminal's colour theme, while the header and legend took the colour names literally, so the legend's `?` (and `✓`, `✗`) had different colours from the tree's. All three now take the same path. The `allow` tag in the tree is a quiet green instead of plain grey, so the collection's action is readable without competing with the loud `deny`.
- **Values in the plain detail dialog stay on one line.** The threshold that moves a long value under its label was sized for the narrow pane beside the trace and applied to the 84-column dialog as well, where a flow like `10.2.0.5:9684 → 142.251.14.102:443` wrapped for no reason. The threshold now follows the layout.
- **A TLS-inspected HTTPS request no longer misses an HTTPS-only rule.** The firewall logs an inspected request as the decrypted inner one: `Protocol` is `HTTP/1.1` and `IsTlsInspected` is true, while an uninspected request says `HTTPS`. The trace compared the bare `HTTP` against the rule's `Https` and marked the protocol as a miss, which on the very rule that terminates TLS was a confident wrong verdict. With the flag the trace treats the row as HTTPS and says so (`HTTPS (TLS inspected, logged as HTTP/1.1)`). Found on real records during the 0.6.0 lab pass; the legacy `properties.msg` format has no such flag, so an HTTP row there gets a `?` on the protocol wherever an inspected HTTPS request would have decided differently, instead of a confident reading either way.
- **The wizard's deploy path works again with a current Azure CLI.** `az eventhubs eventhub create` has defaulted to seven days of retention for a while, and the Basic namespace the wizard creates accepts at most one day, so the deployment stopped at *Creating Event Hub* with `MessagingGatewayBadRequest` and left the namespace behind. The wizard now asks for one day explicitly (the tier's maximum, and enough for a live viewer). Found by driving the wizard against a real subscription; every path it offers (discover with Entra ID and with an existing or newly created SAS rule, enter, paste, deploy with Entra ID and with SAS) now has a live test that creates and removes its own resources.
- **A Data Receiver role on the hub alone no longer counts as missing.** Before connecting with Entra ID the viewer asks ARM which data actions the identity has, and it asked at the namespace. A role assigned on the hub itself, which is what the wizard's deploy path does and what least privilege asks for, is not visible one level up, so a freshly deployed hub ended in *Missing 'Azure Event Hubs Data Receiver' role* although receiving would have worked. The check now runs at the hub's scope, where assignments from the namespace, the resource group and the subscription are inherited and show up as well. The message names both scopes.
- **A failed `az` call in the wizard says why.** Deployment and rule creation used to report `Command [...] returned non-zero exit status 1` and nothing else, because the CLI's reason sits in stderr. The wizard now shows that `ERROR:` line, so a quota, a permission or a bad name is readable on screen instead of having to be reproduced by hand.
- **No traceback from the row timer on quit.** The one-second flush that moves received rows into the table could tick once more while the viewer was shutting down, after the table had been removed, and end the session with `NoMatches: No nodes match '#log-table'`. It now checks that the table is still there before it touches anything and leaves the rows pending otherwise.
