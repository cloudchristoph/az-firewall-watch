...
- **The property bag as proper rows.** The Instance block used to dump `additionalProperties` as `EnableFatFlowLogging=true`. The switches Azure keeps there now have rows of their own: *Fat flow logging* and *Active FTP* (off when not set, which is Azure's default), *DNS flow trace* and the classic DNS proxy and SNAT settings when present. Keys the viewer does not know stay visible under *Additional*, so nothing is lost.
